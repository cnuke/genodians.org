...
#include <tm.h>
namespace Utils {
	struct Seconds { long long value; };

	Seconds        from_rtc(Rtc::Timestamp const &);
...
Utils::Seconds Utils::from_rtc(Rtc::Timestamp const &rtc)
{
	struct tm tm;
	...
	return Seconds { .value = tm_to_secs(&tm) };
}
...
namespace Genodians {
	...
	// XXX move to Utils or better maybe adapt Rtc::Timestamp?
	using Date_string = String<21>;
	Date_string from_rtc(Rtc::Timestamp const &ts, bool iso_8601 = true)
	{
	...
	struct Generator : Interface
	{
		...
		virtual Start_name const name()                       const = 0;
	...
	struct Init : Interface
	{
		...
		Init(Env &env, char const *state_name, char const *config_name)
		:
	...
	struct Config
	{
		struct Child
		{
			...
			static Child from_xml(Xml_node const &node)
			{
				return Child {
					...
					.heartbeat_ms = node.attribute_value("heartbeat_ms", 3000u)
				};
			}
		};
		...
		struct Import
		{
			...
			Child extract;
			Child generate;
			Microseconds sleep_duration;
		};

		Child  lighttpd_config;
		Import import_config;

		static Config update_from_xml(Xml_node const &node)
		{
			// XXX crash and burn on invalid config

			Xml_node const &lighttpd_node = node.sub_node("lighttpd");
			Child const lighttpd_config = Child::from_xml(lighttpd_node);

			Xml_node const &import_node = node.sub_node("import");
			Microseconds const sleep_duration = Microseconds {
				import_node.attribute_value("update_interval_min", 120u) * 60 * 1000'000 };

			Xml_node const &fetchurl_node = import_node.sub_node("fetchurl");
			Child    const fetchurl       = Child::from_xml(fetchurl_node);

			Xml_node const &wipe_node = import_node.sub_node("wipe");
			Child    const wipe       = Child::from_xml(wipe_node);

			Xml_node const &extract_node = import_node.sub_node("extract");
			Child    const extract       = Child::from_xml(extract_node);

			Xml_node const &generate_node = import_node.sub_node("generate");
			Child    const generate       = Child::from_xml(generate_node);
...
struct Genodians::Fetch : Genodians::Generator
{
	Child_state _child_state;

	Fetch(Init::Child_state_registery &registry,
	      Config::Child         const &config)
	:
		_child_state { registry, "fetchurl",
		               Priority  { 0 },
		               config.ram, config.caps }
	{ }

	Start_name const name() const override {
...
struct Genodians::Wipe : Genodians::Generator
{
	Child_state _child_state;

	Wipe(Init::Child_state_registery &registry,
	     Config::Child         const &config)
	:
		_child_state { registry, "wipe",
		               Priority  { 0 },
		               config.ram, config.caps }
	{ }
...
struct Genodians::Extract : Genodians::Generator
{
	Child_state _child_state;

	Extract(Init::Child_state_registery &registry,
	        Config::Child         const &config)
	:
		_child_state { registry, "extract",
		               Priority  { 0 },
		               config.ram, config.caps }
	{ }
...
struct Genodians::Generate : Genodians::Generator
{
	Child_state _child_state;

	Generate(Init::Child_state_registery &registry,
	         Config::Child         const &config)
	:
		_child_state { registry, "generate",
		               Priority  { 0 },
		               config.ram, config.caps }
	{ }
...
struct Genodians::Sleep : Genodians::Generator
{
	Timer::Connection &_timer;
	Timer::One_shot_timeout<Sleep> _sleep_timeout;

	Microseconds     const  _sleep_duration;
	Notify_interface const &_notify;
	...
	Sleep(Timer::Connection &timer,
	      Notify_interface  &notify,
	      Microseconds       sleep_duration)
	:
		_timer          { timer },
		_sleep_timeout  { _timer, *this,
		                 &Genodians::Sleep::_handle_sleep_timeout },
		_sleep_duration { sleep_duration },
		_notify         { notify },
		_triggered      { false }
	{
		_sleep_timeout.schedule(_sleep_duration);
	}

	bool duration_finished() const { return _triggered; }

	Start_name const name() const override
	{
		static Start_name name("sleep");
		return name;
	}
...
struct Genodians::Import : Genodians::Init
{
	Env       &_env;
	Allocator &_alloc;

	Notify_interface &_state_notify;
	...
	Signal_handler<Import> _timeout_handler {
		_env.ep(), *this, &Import::_handle_timeout };

	void _handle_timeout()
	{
		if (_cached_state_report.constructed())
			_cached_state_report->with_xml_node([&] (Xml_node const &node) {
				// XXX implement sleep differently
				Init::_state_update(node); });
	}

	struct Notify : Notify_interface
	{
		Signal_handler<Import> &_handler;

		Notify(Signal_handler<Import> &handler)
		: _handler { handler } { }
	...
	Notify _sleep_timeout_notifier { _timeout_handler };

	void _update_init_config(Xml_generator &xml);

	Config::Import const &_config;

	Date_string _last_update { };
	Date_string _next_update { };
	...
	:
		Init          { env, "import.state",
		                     "import.config" },
		_env          { env },
		_alloc        { alloc },
		_state_notify { notify },
		_timer        { timer },
		_rtc          { rtc },
...
void Genodians::Import::generate_report(Xml_generator &xml) const
{
	if (_cached_state_report.constructed()) {
		/* raw init state */
		xml.append("<!-- raw import init state");
		...
		if (_imports)
			Html::gen_table_body(xml, [&] (Xml_generator &xml) {
				...
				Html::gen_table_key_value_row(xml, Html::String("Import duration"),
				                                   Html::String(_import_duration.value,
				                                                " seconds"));
			...
			xml.node("p", [&] { xml.append("initial import pending…"); });
	});
}
                                     ...
                                     bool            reconfigure_init)
{
	...
	switch (_state) {
	case State::INVALID: [[fallthrough]];
	case State::INIT:
	{
		_import_start = Utils::from_rtc(_rtc.current_time());
		_fetch.construct(Init::child_states,
		                 _config.fetchurl);
	...
	case State::FETCH:
	{
		new_state = _fetch->check(state_node).convert<State>(
			[&] (Generator::Ok ok) {
				...
				_fetch.destruct();
				_wipe.construct(Init::child_states,
				                _config.wipe);
				return State::WIPE;
			},
			[&] (Generator::Error err) {
	...
	case State::WIPE:
	{
		new_state = _wipe->check(state_node).convert<State>(
			[&] (Generator::Ok ok) {
				...
				_wipe.destruct();
				_extract.construct(Init::child_states,
				                   _config.extract);
	...
	case State::EXTRACT:
	{
		new_state = _extract->check(state_node).convert<State>(
			[&] (Generator::Ok ok) {
				...
				_extract.destruct();
				_generate.construct(Init::child_states,
				                    _config.generate);
	...
	case State::GENERATE:
	{
		new_state = _generate->check(state_node).convert<State>(
			[&] (Generator::Ok ok) {
				...
				_generate.destruct();
				_sleep.construct(_timer, _sleep_timeout_notifier,
				                         _config.sleep_duration);

				++_imports;
				Rtc::Timestamp const ts   = _rtc.current_time();
				Seconds        const secs = Utils::from_rtc(ts);
				Seconds        const dur {
					.value = (long long)_config.sleep_duration.value/1'000'000 };

				_import_duration = { secs.value - _import_start.value };

				_last_update = Genodians::from_rtc(ts);
				_next_update = Genodians::from_rtc(from_seconds({ .value = secs.value
				                                                         + dur.value }));
				return State::SLEEP;
			},
			[&] (Generator::Error err) {
	...
	case State::SLEEP:
	{
		if (!_sleep->duration_finished()) {
			new_state = State::SLEEP;
		} else {
			_sleep.destruct();
	...
	if (_update_cached_state_report(state_node))
		_state_notify.notify();

	if (_state == new_state && !reconfigure_init)
		return;

	// TODO evaluate new_exit_value to handle different invalid states

	_state = new_state;
	Init::generate_config([&] (Xml_generator &xml) {
...
void Genodians::Import::_update_init_config(Xml_generator &xml)
{
	...
	if (_sleep.constructed())    _sleep->   generate(xml);
}


struct Genodians::Lighttpd : Genodians::Init
{
	Env       &_env;
	Allocator &_alloc;
	...
	Config::Child const &_config;

	Child_state _child_state;

	Date_string _last_restart { };
	...
	:
		...
		_rtc          { rtc },
		_config       { config },
		_child_state  { Init::child_states, "lighttpd",
		                Priority { 0 }, _config.ram, _config.caps }
	{
...
{
	if (_cached_state_report.constructed()) {
		xml.append("<!-- raw lighttpd init state");
		_cached_state_report->with_xml_node(
			[&] (Xml_node const &node) {
				node.with_raw_content([&] (char const *s, size_t len) {
					xml.append(s, len); }); });
...
struct Genodians::Main
{
	...
	unsigned _status_version = 0;

	Utils::Seconds _uptime { 0 };
	Date_string _last_status_update { };
		...
		Html::gen_section_div(xml, "Overview", [&] (Xml_generator &xml) {
			Html::gen_table_body(xml, [&] (Xml_generator &xml) {
				Html::gen_table_key_value_row(xml,
					Html::String("Status version"), Html::String(_status_version));

				Html::gen_table_key_value_row(xml,
					Html::String("Last update"), _last_status_update);

				Html::gen_table_key_value_row(xml,
					Html::String("Uptime"), Html::String(_uptime.value, " seconds"));
			});
		});
	}
	...
	void _handle_status()
	{
		...
		_uptime = { (long long)_timer.curr_time().trunc_to_plain_ms().value / 1'000u };
