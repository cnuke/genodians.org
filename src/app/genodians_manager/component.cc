...
namespace Utils {
	...
	namespace Html {
		...
		void gen_section_div(Generator &g, char const *name, auto const &fn);
		void gen_table_body(Generator &g, auto const &fn);
		void gen_table_key_value_row(Generator &, Html::String const &, Html::String const &);
	} /* namespace Html */
} /* namespace Utils */
...
void Utils::Html::gen_section_div(Generator &g, char const *name, auto const &fn)
{
	g.node("div", [&] {
		g.attribute("id", name);
		g.node("h3", [&] { g.append_quoted(name); });
		fn(g); });
}


void Utils::Html::gen_table_body(Generator &g, auto const &fn) {
	g.node("table", [&] { g.node("tbody", [&] { fn(g); }); }); }


void Utils::Html::gen_table_key_value_row(Generator &g, Utils::Html::String const &key,
                                                              Utils::Html::String const &value)
{
	g.node("tr", [&] {
		g.node("td", [&] { g.append_quoted(key.string()); });
		g.node("td", [&] { g.append_quoted(value.string()); });
	});
}
...
void Genodians::Import::generate_report(Generator &g) const
{
	Html::gen_section_div(g, "Import", [&] (Generator &g) {
		if (_imports) {
			Html::gen_table_body(g, [&] (Generator &g) {
				Html::gen_table_key_value_row(g, Html::String("Total imports"),
				                                   _imports);
				Html::gen_table_key_value_row(g, Html::String("Import duration"),
				                                   _import_duration);
				Html::gen_table_key_value_row(g, Html::String("Last import"),
				                                   _last_update);
				Html::gen_table_key_value_row(g, Html::String("Next import"),
				                                   _next_update);
			});
			g.node("p", [&] { g.append_quoted("Last durations"); });
			Html::gen_table_body(g, [&] (Generator &g) {
				Html::gen_table_key_value_row(g, Html::String("Fetch"),
				                                   _last_fetch_duration);
				Html::gen_table_key_value_row(g, Html::String("Extract"),
				                                   _last_extract_duration);
				Html::gen_table_key_value_row(g, Html::String("Wipe"),
				                                   _last_wipe_duration);
				Html::gen_table_key_value_row(g, Html::String("Generate"),
		...
		Managed_init::with_cached_state_report([&] (Node const &node) {
			...
			if (node.has_sub_node("child"))
				g.node("p", [&] {
					if (!_imports) g.append_quoted("initial ");
					g.append_quoted("import under way…"); });

			g.node("h4", [&] { g.append_quoted("init-state-report"); });
			g.node("pre", [&] {
				g.attribute("lang", "g");
				g.append_quoted(node); });
		}, [&] { });
	});
}
...
void Genodians::Lighttpd::generate_report(Generator &g) const
{
	Html::gen_section_div(g, "Lighttpd", [&] (Generator &g) {
		Html::gen_table_body(g, [&] (Generator &g) {
			Html::gen_table_key_value_row(g, Html::String("Total restarts"),
			                                   Html::String(_restarts));
			if (_restarts == 0)
				return;

			Html::gen_table_key_value_row(g, Html::String("Last restart"),
			                                   _last_restart);
		});
		Managed_init::with_cached_state_report([&] (Node const &node) {
			g.node("h4", [&] { g.append_quoted("init-state-report"); });
...
struct Genodians::Main
{
	...
	void _generate_nic_router_report(Generator &g, Node const &node)
	{
		auto td_centered = [&] (Generator &g, auto value) {
			g.node("td", [&] {
				g.attribute("style", "text-align:center");
				g.append_quoted(value); });
		};

		auto td_right = [&] (Generator &g, auto value) {
			g.node("td", [&] {
				g.attribute("style", "text-align:right");
				g.append_quoted(value); });
		};
		...
		g.node("table", [&] {
			...
			g.node("tbody", [&] {
				node.for_each_sub_node("domain", [&] (Node const &domain) {
					g.node("tr", [&] {
						g.node("td", [&] {
							g.append_quoted(domain.attribute_value("name", Html::String())); });
						td_right(g, domain.attribute_value("rx_bytes", 0ull));
						td_right(g, domain.attribute_value("tx_bytes", 0ull));
					}); }); });
		});
	}
	...
	void _generate_report(Generator &g)
	{
		Html::gen_section_div(g, "Overview", [&] (Generator &g) {
			Html::gen_table_body(g, [&] (Generator &g) {
				Html::gen_table_key_value_row(g,
				...
				_nic_router_state_rom.with_node([&] (Node const &node) {
					_generate_nic_router_report(g, node);
				});
			});
		});
	}
	...
	void _handle_status()
	{
		...
		_status_reporter.generate([&] (Generator &g) {

			g.node("head", [&] {
				g.node("meta", [&] {
					g.attribute("charset", "utf-8"); });
				g.node("title", [&] {
					g.append_quoted("Genodians Status Overview"); });
			});

			g.node("body", [&] {

				g.attribute("style", "font-family:monospace");

				_generate_report(g);

				_lighttpd.generate_report(g);
				_import.  generate_report(g);
			});
		});
